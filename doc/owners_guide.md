...
- **PID注意**: systemd の `MainPID` が `run.sh` の場合でも、`reload.sh` は子の SBCL PID を自動検出して HUP を送ります。
- **確認**: `journalctl -u swimmy-brain -n 50 --no-pager | rg 'Hot Reload|LOADER|ASDF'`
- **効果**: リードタイムの大幅短縮。
...
### ⚡ 即時ステータス送信（3通貨ペア）
`STATUS_NOW` は **Brain 5555(PULL)** に `PUSH` で送る。`PUB` では届かない。
...
# 送信後の確認（鮮度秒数が数秒〜十数秒に更新される）
rg -n "USDJPY 状況レポート|EURUSD 状況レポート|GBPUSD 状況レポート|データ鮮度" logs/notifier.log | tail -n 20
```
...
├─────────────────────────────────────────────────────┤
│  BRAIN (Lisp)           Port 5555 (PULL) / 5556 (PUB) │
