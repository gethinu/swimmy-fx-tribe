...
- 残っているのは reverse-engineering そのものではなく、移植後の検証と整理です:
  - Strategy Tester 実測の実行枠確保
  - `timeframe=3600` 個体の custom timeframe 前提の扱い整理
  - stale source を将来どこまで掃除するかの code cleanup 判断
- 速く MQ5 化を進めるなら、先に named Legend set から触るのが正解です。

## 6. 推奨する次の手順

MQ5 化そのものと offline compile / audit は完了したので、freeze 後の残タスクは次です。

1. `tools/mt5_inventory_tester.py` を使って isolated Strategy Tester 実測を回す。
2. `batch-4` の 3 本は dropped 扱いを維持し、active comparison からは外す。
3. runtime が凍結のままなら、MQ5 / doc 一式をコミット単位で整理し、再開時に tester から再開できる形へ固める。
