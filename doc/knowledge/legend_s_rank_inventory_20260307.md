...
- broker variance の proxy probe も実施したが、未解決のままです:
  - artifact:
...
- したがって、現時点で残っている実務上の blocker は broker variance だけです。
...
4. broker variance を本当に閉じるなら、第二 broker snapshot か source-label 付き offline export を先に用意する。
