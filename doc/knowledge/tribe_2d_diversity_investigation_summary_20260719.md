...
**これは何か.** 2026-07-18〜21 に走った tribe 多様性エンジン探求 **11 段**の集約・整合レコード（⑪ cross-pair relative-value 4 路線を 2026-07-21 追加）。個別 doc は各段の
...
| 案 | 実体 | headroom |
|---|---|---|
| **native に 2-window robust な新 instrument / 新データ軸（残る唯一の本命）** | 現4銘柄の *派生でなく*、単一系列・単脚 cost で intrinsic に両レジーム安定なエッジを持つ新 instrument（別アセットクラス＝metals/index/rates 等の異なる regime dynamics） | **中** — ⑪ が cross-pair 派生（2-leg も 1-leg も）を打ち止め: hedge=robust だが 2-leg cost、単脚=安いが de-hedge で robustness 喪失＝同一ノブ反対向き。⑦ data-wall とも整合（非robust 機序への data 追加は無駄）。要は「clean M1 のある native-robust instrument」だが tribe は未保有 |
...
**今すぐ ship 安全なもの（コード）:** B-4/B-5/B-3/seed-escape/**session-軸 scorer+休眠 genome hook**/**hold-to-barrier 軸 scorer（`hold_mode`）**/**⑪ の vol-regime（`vol_*`）・multi-TF confluence（`htf_*`）・single-leg RV（`rvspread`/`rv_*`/`--aux`）scorer 軸** は全て可逆・OFF byte-identical（`73394ee4…`）・honest floor 中立・offline 実証済。
...
| ⑪ | 物差し拡張③ cross-pair relative-value 4 路線（最終） | `tribe_2d_relative_value_multiaxis_20260721.md` | (this run) |
