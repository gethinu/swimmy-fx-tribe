...
//|               Historical S extraction batch-2 port               |
//| Original: Bred-Bred--794-Gen32-N3980040593-767                  |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "Historical S batch-2: Bred-Bred--794-Gen32-N3980040593-767"

#include <Trade/Trade.mqh>

// Original row uses timeframe=3600, which is not a native MT5 timeframe.
// Stored indicators mention PSAR, but the executable entry expression does not use it.
input string           InpSymbol = "USDJPY";
input ENUM_TIMEFRAMES  InpTimeframe = PERIOD_CURRENT;
input double           InpLots = 0.01;
input ulong            InpMagic = 9307502;
input int              InpSlippagePoints = 30;
input double           InpStopLossPrice = 0.55771023;
input double           InpTakeProfitPrice = 0.67570390;
...
bool OpenManagedPosition(const bool is_buy) {
   ...
   if(is_buy) return g_trade.Buy(InpLots, symbol, 0.0, sl, tp, "Bred-Bred--794-Gen32-N3980040593-767");
   return g_trade.Sell(InpLots, symbol, 0.0, sl, tp, "Bred-Bred--794-Gen32-N3980040593-767");
}
