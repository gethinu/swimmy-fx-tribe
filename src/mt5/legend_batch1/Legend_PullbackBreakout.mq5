...
#property description "LEGEND batch-1: Pullback-Breakout"
...
bool OpenManagedPosition(const bool is_buy) {
   ...
   if(is_buy) return g_trade.Buy(InpLots, symbol, 0.0, sl, tp, "Pullback-Breakout");
   return g_trade.Sell(InpLots, symbol, 0.0, sl, tp, "Pullback-Breakout");
}
...
void OnTick() {
   ...
   double prev_ema = ema_buf[1];
   double curr_rsi = rsi_buf[0];

   bool long_entry = CrossAbove(prev_close, prev_ema, curr_close, curr_ema) && curr_rsi > InpLongRSIMin;
   bool short_entry = CrossBelow(prev_close, prev_ema, curr_close, curr_ema) && curr_rsi < InpShortRSIMax;
   bool long_exit = CrossBelow(prev_close, prev_ema, curr_close, curr_ema);
   bool short_exit = CrossAbove(prev_close, prev_ema, curr_close, curr_ema);
