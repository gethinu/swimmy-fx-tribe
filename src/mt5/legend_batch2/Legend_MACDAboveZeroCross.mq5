...
#property description "LEGEND batch-2: MACD-Above-Zero-Cross"
...
bool OpenManagedPosition(const bool is_buy) {
   ...
   if(is_buy) return g_trade.Buy(InpLots, symbol, 0.0, sl, tp, "MACD-Above-Zero-Cross");
   return g_trade.Sell(InpLots, symbol, 0.0, sl, tp, "MACD-Above-Zero-Cross");
}
...
void OnTick() {
   ...
   bool long_entry = (curr_main > 0.0 && CrossAbove(prev_main, prev_signal, curr_main, curr_signal));
   bool short_entry = (curr_main < 0.0 && CrossBelow(prev_main, prev_signal, curr_main, curr_signal));
