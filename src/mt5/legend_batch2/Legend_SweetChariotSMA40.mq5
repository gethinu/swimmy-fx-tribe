...
#property description "LEGEND batch-2: Sweet-Chariot-SMA-40"
...
bool LoadSeries(double &close_buf[], double &sma_buf[]) {
...
bool OpenManagedPosition(const bool is_buy) {
   ...
   if(is_buy) return g_trade.Buy(InpLots, symbol, 0.0, sl, tp, "Sweet-Chariot-SMA-40");
   return g_trade.Sell(InpLots, symbol, 0.0, sl, tp, "Sweet-Chariot-SMA-40");
}
...
int OnInit() {
   ...
   g_sma_handle = iMA(symbol, InpTimeframe, InpSMAPeriod, 0, MODE_SMA, PRICE_CLOSE);
   if(g_sma_handle == INVALID_HANDLE) return INIT_FAILED;
...
void OnTick() {
   ...
   double close_buf[];
   double sma_buf[];
   if(!LoadSeries(close_buf, sma_buf)) return;

   double curr_close = close_buf[0];
   double prev_close = close_buf[1];
   double curr_sma = sma_buf[0];
   double prev_sma = sma_buf[1];

   bool long_entry = CrossAbove(prev_close, prev_sma, curr_close, curr_sma);
   bool short_entry = CrossBelow(prev_close, prev_sma, curr_close, curr_sma);
   bool long_exit = CrossBelow(prev_close, prev_sma, curr_close, curr_sma);
   bool short_exit = CrossAbove(prev_close, prev_sma, curr_close, curr_sma);
