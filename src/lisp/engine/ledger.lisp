          ...
          (with-open-file (out tmp-path 
            ...
            (let* ((arms-data (loop for arm in *arms*
                   ...
                   (state-obj (list :version "1.0"
                                    ...
                                    :tribe-consensus (if (boundp '*tribe-consensus*) *tribe-consensus* nil)
                                    :tribe-direction (if (boundp '*tribe-direction*) *tribe-direction* nil)
          ...
          (let ((state-obj (read in nil nil)))
            (when state-obj
              (let ((arms-data (getf state-obj :arms))
                    ...
                    (tribe-consensus (getf state-obj :tribe-consensus))
                    (tribe-direction (getf state-obj :tribe-direction :missing))
                ...
                (when (boundp '*last-swarm-consensus*)
                  (setf *last-swarm-consensus* (or last-swarm-consensus 0.0)))
                (when (boundp '*tribe-consensus*)
                  (setf *tribe-consensus* (or tribe-consensus 0.0)))
                (when (and (boundp '*tribe-direction*) (not (eq tribe-direction :missing)))
                  (setf *tribe-direction* tribe-direction))
