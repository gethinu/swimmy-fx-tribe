          ...
          (progn
            (let* ((progress (get-goal-progress))
                   ...
                   (payload `((schema_version . 1)
                              ...
                              (tribes . ((hunters . ((direction . ,(if hunter-sig (symbol-name (getf hunter-sig :direction)) "HOLD"))
                                                     (confidence . ,(* 100 (or (and hunter-sig (getf hunter-sig :confidence)) 0)))
                                                     (reason . "MACD+ADX+Kalman")))
                                         (shamans . ((direction . ,(if shaman-sig (symbol-name (getf shaman-sig :direction)) "HOLD"))
                                                     (confidence . ,(* 100 (or (and shaman-sig (getf shaman-sig :confidence)) 0)))
                                                     (reason . "RSI+BB Mean Reversion")))
                                         (breakers . ((direction . ,(if breaker-sig (symbol-name (getf breaker-sig :direction)) "HOLD"))
                                                      (confidence . ,(* 100 (or (and breaker-sig (getf breaker-sig :confidence)) 0)))
                                                      (reason . "Bollinger Breakout")))
                                         (raiders . ((direction . ,(if raider-sig (symbol-name (getf raider-sig :direction)) "HOLD"))
                                                     (confidence . ,(* 100 (or (and raider-sig (getf raider-sig :confidence)) 0)))
                                                     (reason . "EMA Scalp+Kalman")))))
                              (tribe_consensus . ((direction . ,(if (boundp '*tribe-direction*) (symbol-name (or *tribe-direction* :hold)) "HOLD"))
