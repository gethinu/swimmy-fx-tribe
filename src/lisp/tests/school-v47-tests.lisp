...
(deftest test-pip-size-by-symbol
  "EV-First: Test pip size mapping for bid/ask to pips conversion"
  (assert-true (< (abs (- 1.0d-4 (swimmy.school::get-pip-size "EURUSD"))) 1e-10))
  (assert-true (< (abs (- 1.0d-4 (swimmy.school::get-pip-size "GBPUSD"))) 1e-10))
  (assert-true (< (abs (- 1.0d-2 (swimmy.school::get-pip-size "USDJPY"))) 1e-10)))

(deftest test-spread-pips-from-bid-ask
  "EV-First: Test spread-to-pips conversion"
  ;; EURUSD: 1 pip = 0.0001
  (assert-true (< (abs (- 2.0 (swimmy.school::spread-pips-from-bid-ask "EURUSD" 1.1000 1.1002))) 1e-3))
  ;; USDJPY: 1 pip = 0.01
  (assert-true (< (abs (- 2.0 (swimmy.school::spread-pips-from-bid-ask "USDJPY" 150.00 150.02))) 1e-3)))

(deftest test-cost-pips-defaults
  "EV-First: Test default cost pips (spread only)"
  (assert-true (< (abs (- 2.0 (swimmy.school::calculate-cost-pips "EURUSD" 1.1000 1.1002))) 1e-3))
  (assert-true (< (abs (- 2.0 (swimmy.school::calculate-cost-pips "USDJPY" 150.00 150.02))) 1e-3)))
