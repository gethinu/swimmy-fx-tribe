...
(defun evaluate-strategy-signal (strat history)
  ...
  (when (and history (> (length history) 100))
    (let* ((indicators (strategy-indicators strat))
           ...
           ;; Use a temporary variable for the loop result to ensure clean scope
           (base-bindings 
            (loop for ind in indicators
                  append (let ((type (car ind)) (p (cdr ind))
                            ...
                            (case type
                              ...
                              (ichimoku (multiple-value-bind (tenkan kijun sa sb) (ind-ichimoku (first p) (second p) (third p) history)
                                          `((,(intern "TENKAN" pkg) ,tenkan)
                                            (,(intern "KIJUN" pkg) ,kijun)
                                            (,(intern "SENKOU-A" pkg) ,sa) 
                                            (,(intern "SENKOU-B" pkg) ,sb))))
                              (donchian (multiple-value-bind (upper lower mid) (ind-donchian (car p) history)
