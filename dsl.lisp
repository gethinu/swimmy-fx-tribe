  ...
  (when (>= (length history) n)
    (let* ((k (/ 2 (+ n 1)))
      ...
      (dolist (c (reverse (subseq history 0 n)))
        (setf ema (+ (* (candle-close c) k) (* ema (- 1 k)))))
      ema)))
...
(defun ind-rsi (n history)
  (when (>= (length history) (1+ n))
    (let ((gains 0) (losses 0))
      ...
      (if (zerop losses) 100 (- 100 (/ 100 (1+ (/ gains (max 0.00001 losses)))))))))
...
(defun ind-bb (n dev history)
  (when (>= (length history) n)
    (let* ((sma (ind-sma n history))
           ...
           (variance (/ (reduce #'+ (mapcar (lambda (x) (expt (- x sma) 2)) closes)) n))
...
(defun ind-stoch (k-n d-n history)
  (when (>= (length history) k-n)
    (let* ((closes (subseq history 0 k-n))
           ...
           (k (if (= high low) 50 (* 100 (/ (- curr low) (max 0.00001 (- high low)))))))
      k)))

(defun ind-cci (n history)
  (when (>= (length history) n)
    (let* ((prices (mapcar (lambda (c) (/ (+ (candle-high c) (candle-low c) (candle-close c)) 3)) (subseq history 0 n)))
           (tp (first prices))
           (sma (/ (reduce #'+ prices) n))
           (md (/ (reduce #'+ (mapcar (lambda (x) (abs (- x sma))) prices)) n)))
      (if (zerop md) 0 (/ (- tp sma) (* 0.015 md))))))
...
(defparameter *allowed* '(+ - * / < > <= >= = and or not if 
                          ...
                          cross-above cross-below defstrategy sma ema rsi macd bb stoch atr cci close high low open))
