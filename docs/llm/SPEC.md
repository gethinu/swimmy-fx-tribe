...
- **ランク体系**: 
  ...
  - **Rank Evidence Semantics**: rank floor / staged gate に使う `TradeEvidence` は `backtest_trade_evidence + paper_forward_trade_evidence + live_trade_evidence` を正本とする。`shadow_trade_evidence` は breakdown / 補助診断には残すが、A/S floor の直接充足には使わない。
  - **Legend**: 不老不死（外部導入戦略）
...
- **実運用 Go/No-Go（Rankと分離）**:
  ...
  - rank evidence と forward evidence は別契約。forward は paper-forward canonical を優先し、rank floor は `SHADOW` を直接カウントしない。
...
| :--- | :--- | :--- | :--- |
| **S-Rank** | Verified Elite | Sharpe ≥ 0.75<br>PF/WR/MaxDD は S基準（PF≥1.70/WR≥50%/MaxDD<10%）<br>RankTradeEvidence ≥ 100 | **CPCV + Common Stage2**<br>- CPCV pass_rate ≥ 70%<br>- CPCV median MaxDD < 12%<br>- Common Stage2（MC prob_ruin / DryRun slippage） |
| **A-Rank** | Pro | Sharpe ≥ 0.45<br>PF ≥ 1.30<br>WR ≥ 43%<br>MaxDD < 16%<br>RankTradeEvidence ≥ 50 | **OOS + Common Stage2**<br>- OOS Sharpe ≥ 0.35 |
...
- **Evidence Breakdown（rank telemetry/report）**: `backtest_trade_evidence` / `paper_forward_trade_evidence` / `shadow_trade_evidence` / `live_trade_evidence` を出し、`trade_evidence` は rank判定に使う `RankTradeEvidence` を指す。
