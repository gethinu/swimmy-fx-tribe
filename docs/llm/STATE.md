...
- **V50.8 runtime selection evidence-aware化（2026-03-03）**:
  - `process-category-trades` の同時signal選定を `raw cached Sharpe` 単独の GLOBAL BEST から evidence-aware scoring へ更新。
  - `deployment gate` / `live-edge` / `evidence-adjusted quality` / `recent stability` / `diversification penalty` を同時評価し、order flow は単一 winner のまま維持。
  - telemetry に `selected/rejected` 理由を追加し、選定経路の監査性を強化。
- **V50.8 EURUSD専用MT5最適化 実行結果（2026-03-03 JST）**:
  - 専用run `ih_opt_eurusd_monthly3_20260301_012747` を `Symbol=EURUSD` + `ExpertParameters=InstitutionalHunterEA_OptimizeMonthly3_EURUSD.set` で実行し、最適化完了を確認。
  - 成果物を回収:
    - `data/reports/mt5/ih_opt_eurusd_monthly3_20260301_012747.xml`
    - `data/reports/mt5/ih_opt_eurusd_monthly3_20260301_012747.forward.xml`
    - `data/reports/mt5/ih_opt_eurusd_monthly3_20260301_012747.summary.json`
    - `data/reports/mt5/ih_opt_eurusd_monthly3_20260301_012747.monthly3target.decision.json`
    - `data/reports/mt5/ih_opt_eurusd_monthly3_20260301_012747.monthly5target.decision.json`
  - 最良候補（forward）実測: `monthly_cagr_pct=5.9136`, `trades=21`, `profit_factor=2.0606`, `equity_dd_pct=45.2789`。
  - 判定: `monthly3target/monthly5target` とも `accept_for_promotion=false`。主因は `drawdown_above_max`（DD上限12%超過）。
  - 通信ポート/メッセージ契約は変更しないため `INTERFACES.md` 更新は不要。
- **Legend MT5 broader-retune follow-up（2026-03-07 JST）**:
  - narrow bounded walk-forward の追加検証として、次の artifact を生成:
  ...
  - 実測:
    - `legend-macd-above-zero-cross` (`wide`): `folds=3/3`, `forward_profit_total=116.34`, `mean_pf=2.9664`, `trades=46`
    - `legend-pullback-breakout` (`wide`): `folds=3/3`, `forward_profit_total=213.03`, `mean_pf=1.6812`, `trades=389`
    - `historical-s-bred940-trend-core` (`medium` fallback): `folds=3/3`, `forward_profit_total=237.99`, `mean_pf=1.8208`, `trades=141`
  - 判定:
...
- **Legend canonical-source alignment（2026-03-08 JST）**:
  - reviewed MT5 port の behavioral source-of-truth は `strategies_v3.lisp` と `data/optimized_timeframes.json` とする。
...
- **V50.8 M15/H1 trial judge 初回同期（2026-03-03）**:
  - `trial_v2_20260303_v50_8_30d_m15` / `trial_v2_20260303_v50_8_30d_h1` を `xau_autobot_trial_v2_eval.sh` で評価。
  - 両runとも `verdict=INVALID_TRIAL`（`decision_mode=monthly_only`, `window_days=30.0`）。
  - root cause は `after_magic_filter=0` かつ `after_comment_prefix_filter=0`（run一致約定なし）。
  - 含意: `M15/H1` は config/判定連携は正常で、次アクションは secondary loop の実運用差し替え。
- **V50.8 secondary M15差し替え（2026-03-03）**:
  - `current_run_r2` を `trial_v2_20260303_v50_8_30d_m15_live`（`magic=560186`）へ更新し、`M15` live loop を起動。
  - ただし eval 実行中の live-loop guard self-heal により `r2v_m5` が再起動し、loop 構成は `m20 + m15 + r2v_m5` の3本へ再変動。
  - `current_run.json` は外部更新で `trial_v2_20260303_primary_r2v_m5_48h` にドリフトしたため、primary 正本の再固定が必要。
- **V50.8 非M5 3run再固定（2026-03-03）**:
  - `current_run*.json` を `primary=H1 / secondary=M15 / explore=M20` へ再固定。
    - `primary=trial_v2_20260303_v50_8_30d_h1_live`
    - `secondary=trial_v2_20260303_v50_8_30d_m15_live`
    - `explore=trial_v2_20260303_r3_m20_e10_live`
  - `xau_autobot_live_loop_guard.py --include-r3 --dry-run` で `status=HEALTHY` を確認（expected `h1/m15/m20` は各1本）。
  - ただし `xau_autobot.armada_mt5_20260303p1_pandajiro_01.json` は `unmanaged_keys` として別系統で並行稼働（分離運用）。
  - 3run再評価（eval guard無効）では全run `verdict=INVALID_TRIAL`、`watchdog.decision=CONTINUE`、運用監査は `INSUFFICIENT_DATA`。
  - 主要成果物:
  ...
  - 通信ポート/メッセージ契約は変更しないため `INTERFACES.md` 更新は不要。
- **V50.8 Armada実運用同期 + refresh #2（2026-03-03）**:
  - `P10-2` の実運用投入を継続し、Armada run を明示評価:
  ...
  - 同期実行:
    - `xau_autobot_trial_v2_eval.sh`（`WATCHDOG=1`, `LIVE_LOOP_GUARD=0`）で judge 更新。
    - `xau_autobot_operational_audit.py --run-id-filter trial_v2_armada_20260303p1_pandajiro_1 --days 3` で監査更新。
  - 最新実測:
    - Armada: `verdict=INVALID_TRIAL`, `closed_positions=0`, `monthly_return_pct=0.0`, `audit_status=INSUFFICIENT_DATA`
    - 非M5 3run（`H1/M15/M20`）も再評価し、全run `monthly_return_pct=0.0` / `closed_positions=0`
  - 成果物:
    - `data/reports/v50_8_run_refresh_20260303_0109_h1m15m20_armada.json`
    - `data/reports/xau_autobot_operational_audit_20260303_{h1_live,m15_live,r3_m20_e10_live,armada_pandajiro}_refresh2_now0109.json`
  - 通信ポート/メッセージ契約は変更しないため `INTERFACES.md` 更新は不要。
...
- **Backtest pending 上限境界契約（2026-03-03）**:
  - root cause: dispatch throttle が `pending >= MAX_PENDING` で発火し、`pending == MAX_PENDING` ちょうどで送信が完全停止する境界張り付き（`Queued throttled request` 連発）を確認。
  - 契約:
    - pending throttle は **`pending > MAX_PENDING`** の超過時のみ発火する。
    - `pending == MAX_PENDING` は許容し、flush/send 経路を止めない。
  - 目的: cap境界での恒久的な送信停滞を防ぎ、結果受信に合わせた自然減圧を維持する。
  - interface 変更なし（`INTERFACES.md` 更新不要）。
- **V50.7 B1R探索スナップショット（2026-02-24）**:
  ...
  - `V50.8-P5` 追補（2026-02-25, 収益改善ラウンド1・再最適化）:
    ...
    - `V50.8-P5.94` 追補（2026-03-01, option2 monitor-only refresh #15）:
      - 監視評価（起動停止なし）:
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260228_m20_e10_shadow14d_refresh2_now1529.json`
        - `xau_autobot_operational_audit_20260228_h4_stable_tfscan_refresh5_now1529.json`
        - `xau_autobot_operational_audit_20260228_m20_e10_c2_refresh5_now1529.json`
        - 3runとも `status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260227_h4_stable_tfscan.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260301_0031_e10shadow_h4stable_e10c2.json`
      - 判定:
        - 本線 `e10_shadow14d` の `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
    - `V50.8-P5.95` 追補（2026-03-01, option2 monitor-only refresh #16）:
      - 監視評価（起動停止なし）:
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260228_m20_e10_shadow14d_refresh3_now1701.json`
        - `xau_autobot_operational_audit_20260228_h4_stable_tfscan_refresh6_now1701.json`
        - `xau_autobot_operational_audit_20260228_m20_e10_c2_refresh6_now1701.json`
        - 3runとも `status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260227_h4_stable_tfscan.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260301_0202_e10shadow_h4stable_e10c2.json`
      - 判定:
        - 本線 `e10_shadow14d` の `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
    - `V50.8-P5.96` 追補（2026-03-01, option2 monitor-only refresh #17）:
      - 監視評価（起動停止なし）:
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260228_m20_e10_shadow14d_refresh4_now1708.json`
        - `xau_autobot_operational_audit_20260228_h4_stable_tfscan_refresh7_now1708.json`
        - `xau_autobot_operational_audit_20260228_m20_e10_c2_refresh7_now1708.json`
        - 3runとも `status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260227_h4_stable_tfscan.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260301_0210_e10shadow_h4stable_e10c2.json`
      - 判定:
        - 本線 `e10_shadow14d` の `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
    - `V50.8-P5.97` 追補（2026-03-01, option2 monitor-only refresh #18）:
      - 監視評価（起動停止なし）:
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260301_m20_e10_shadow14d_refresh5_now1439.json`
        - `xau_autobot_operational_audit_20260301_h4_stable_tfscan_refresh8_now1439.json`
        - `xau_autobot_operational_audit_20260301_m20_e10_c2_refresh8_now1439.json`
        - 3runとも `status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260227_h4_stable_tfscan.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260301_2340_e10shadow_h4stable_e10c2.json`
      - 判定:
        - 本線 `e10_shadow14d` の `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
        - `h4_stable_tfscan` と `e10_c2` は watchdog `NO_GO_ROTATE_REQUIRED` で zero-trade 24h 条件に到達。
    - `V50.8-P5.98` 追補（2026-03-01, option2 monitor-only refresh #19）:
      - 監視評価（起動停止なし）:
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260301_m20_e10_shadow14d_refresh5_now1441.json`
        - `xau_autobot_operational_audit_20260301_h4_stable_tfscan_refresh8_now1441.json`
        - `xau_autobot_operational_audit_20260301_m20_e10_c2_refresh8_now1441.json`
        - 3runとも `status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260227_h4_stable_tfscan.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260301_2342_e10shadow_h4stable_e10c2.json`
      - 判定:
        - 本線 `e10_shadow14d` の `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
        - `h4_stable_tfscan` と `e10_c2` は watchdog `NO_GO_ROTATE_REQUIRED` で zero-trade 24h 条件に到達。
    - `V50.8-P5.99` 追補（2026-03-02, option2 monitor-only refresh #20）:
      - 監視評価（起動停止なし）:
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260302_m20_e10_shadow14d_refresh6_now0013.json`
        - `xau_autobot_operational_audit_20260302_h4_stable_tfscan_refresh9_now0013.json`
        - `xau_autobot_operational_audit_20260302_m20_e10_c2_refresh9_now0013.json`
        - 3runとも `status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260227_h4_stable_tfscan.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260302_0914_e10shadow_h4stable_e10c2.json`
      - 判定:
        - 本線 `e10_shadow14d` の `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
        - `primary/secondary/explore` 全て watchdog `NO_GO_ROTATE_REQUIRED`（zero-trade 24h 条件）。
    - `V50.8-P5.100` 追補（2026-03-02, watchdog rotation execution）:
      - ローテーション実施:
        - `secondary` を `trial_v2_20260302_r2_rotate_r2v_m5_001402`（`tools/configs/xau_autobot.trial_v2_20260227_v50_8_30d_r2v_m5.json`）へ切替。
        - `explore` を `trial_v2_20260302_r3_rotate_alpha4r2_001402`（`tools/configs/xau_autobot.trial_v2_20260227_alpha4_r2_liveprobe.json`）へ切替。
      - live loop 収束:
        - `tools/xau_autobot_live_loop_guard.py --include-r3` で新 `r2/r3` を再起動。
        - 旧run（`h4_stable_tfscan`, `e10_c2`）を停止後、再度 guard 実行で `expected_config_counts_post=1/1/1`（`unmanaged_keys=[]`）を確認。
      - 監視評価:
        - `e10_shadow14d`（primary）: `verdict=INVALID_TRIAL`, `window_days=1.37264636625`, `closed_positions=0`, `watchdog.window_hours=32.94351279`, `watchdog.decision=NO_GO_ROTATE_REQUIRED`
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260302_m20_e10_shadow14d_refresh7_now0014.json`
        - `xau_autobot_operational_audit_20260302_r2v_m5_refresh5_now0014.json`
        - `xau_autobot_operational_audit_20260302_alpha4_r2_liveprobe_refresh2_now0014.json`
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_trial_judge_trial_v2_20260302_r3_rotate_alpha4r2_001402.json`
        - `v50_8_dual_run_status_20260226.json`
        - `v50_8_dual_run_status_20260227.json`
        - `v50_8_monthly_decision_20260225.json`
        - `v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_gap_analysis_20260226_runtime_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260302_0915_e10shadow_r2vm5_a4r2.json`
      - 判定:
        - ユーザー指示どおり `secondary/explore` は rotation 済み。
        - 本線 `e10_shadow14d` の `decision=NO_GO` は継続。
    - `V50.8-P5.101` 追補（2026-03-02, option2 monitor-only refresh #21）:
      - 監視評価（起動停止なし）:
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260302_m20_e10_shadow14d_refresh8_now1125.json`（`status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`）
        - `xau_autobot_operational_audit_20260302_r2v_m5_refresh6_now1125.json`（`status=FAIL`, `gap_reject_rate=1.0`, `runtime_metrics_source=journal`）
        - `xau_autobot_operational_audit_20260302_alpha4_r2_liveprobe_refresh3_now1125.json`（`status=FAIL`, `gap_reject_rate=1.0`, `runtime_metrics_source=journal`）
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_trial_judge_trial_v2_20260302_r3_rotate_alpha4r2_001402.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260302_1126_e10shadow_r2vm5_a4r2.json`
      - 判定:
        - `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
        - `primary` は watchdog `NO_GO_ROTATE_REQUIRED` 継続。
        - `secondary/explore` は watchdog `CONTINUE` だが、audit は `FAIL`（`gap_reject_rate=1.0`）。
    - `V50.8-P5.102` 追補（2026-03-02, option2 monitor-only refresh #22 / live-loop recovery）:
      - 監視評価（起動停止なし）:
        ...
        - `python3 tools/xau_autobot_live_loop_guard.py --include-r3` 実行後は `status=HEALTHY`（trial 3本復旧）。
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260302_m20_e10_shadow14d_refresh7_now1354.json`（`status=INSUFFICIENT_DATA`, `runtime_metrics_source=journal`）
        - `xau_autobot_operational_audit_20260302_r2_rotate_r2v_m5_refresh1_now1354.json`（`status=FAIL`, `gap_reject_rate=1.0`, `signal_eval_count=140`）
        - `xau_autobot_operational_audit_20260302_r3_rotate_alpha4r2_refresh1_now1354.json`（`status=FAIL`, `gap_reject_rate=1.0`, `signal_eval_count=140`）
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_trial_judge_trial_v2_20260302_r3_rotate_alpha4r2_001402.json`
        ...
        - `v50_8_monthly_decision_20260225.json`
        - `v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_gap_analysis_20260302_runtime_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260302_2255_e10shadow_r2rotate_r3rotate.json`
      - 判定:
        - run欠損（trial missing）は復旧済み。
        - `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続。
        - `primary` は watchdog `NO_GO_ROTATE_REQUIRED`、`secondary/explore` は watchdog `CONTINUE` だが audit `FAIL`。
    - `V50.8-P5.103` 追補（2026-03-03, option2 monitor-only refresh #23）:
      - 監視評価（起動停止なし）:
        - `tools/xau_autobot_live_loop_guard.py --include-r3 --dry-run` は `status=HEALTHY`（trial 3本稼働、`unmanaged=tuned_auto_active` 残存）。
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260303_m20_e10_shadow14d_refresh8_now0050.json`（`status=FAIL`, `gap_reject_rate=1.0`）
        - `xau_autobot_operational_audit_20260303_r2_rotate_r2v_m5_refresh2_now0050.json`（`status=WARN`, `gap_reject_rate=0.7003891050583657`, `tp_sl_ratio=1.1`, `expectancy=61.190476190476375`）
        - `xau_autobot_operational_audit_20260303_r3_rotate_alpha4r2_refresh2_now0050.json`（`status=FAIL`, `gap_reject_rate=0.7159533073929961`, `tp_sl_ratio=0.3333333333333333`, `expectancy=-1136.5`）
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_trial_judge_trial_v2_20260302_r3_rotate_alpha4r2_001402.json`
        ...
        - `v50_8_gap_analysis_20260302_runtime_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260303_0950_e10shadow_r2rotate_r3rotate.json`
      - 判定:
        - `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続（primary判定軸）。
        - `secondary` は run一致実績が出て `NO_GO`（`INVALID_TRIAL` 脱出）。
    - `V50.8-P5.104` 追補（2026-03-03, option2 monitor-only refresh #24）:
      - 監視評価（起動停止なし）:
        - `tools/xau_autobot_live_loop_guard.py --include-r3` は `status=HEALTHY`（managed trial 3本稼働）。
        - unmanaged loop として `xau_autobot.tuned_auto_active.json` は残存（停止は未実施）。
      ...
      - 監査更新:
        - `xau_autobot_operational_audit_20260303_m20_e10_shadow14d_refresh9_now0052.json`（`status=FAIL`, `gap_reject_rate=1.0`）
        - `xau_autobot_operational_audit_20260303_r2v_m5_refresh7_now0052.json`（`status=WARN`, `gap_reject_rate=0.7003891050583657`, `tp_sl_ratio=1.1`, `expectancy=61.190476190476375`）
        - `xau_autobot_operational_audit_20260303_alpha4_r2_liveprobe_refresh4_now0052.json`（`status=FAIL`, `gap_reject_rate=0.7159533073929961`, `tp_sl_ratio=0.3333333333333333`, `expectancy=-1136.5`）
      - 集約更新:
        - `v50_8_trial_judge_trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`
        - `v50_8_trial_judge_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_trial_judge_trial_v2_20260302_r3_rotate_alpha4r2_001402.json`
        ...
        - `v50_8_gap_analysis_20260226_runtime_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`
        - `v50_8_monitor_only_status_20260227.json`
        - `v50_8_run_refresh_20260303_0052_e10shadow_r2vm5_a4r2.json`
      - 判定:
        - `v50_8_monthly_decision_20260225.json` は `decision=NO_GO` 継続（primary判定軸）。
        - `secondary/explore` は `NO_GO`（`INVALID_TRIAL` ではない）を維持。
...
- **2026-03-03**: V50.8-P5.108（option2 monitor-only refresh #27 / run drift sync + armada observe）を反映。外部更新で `current_run=trial_v2_20260303_v50_8_30d_h1_live`、`current_run_r2=trial_v2_20260303_v50_8_30d_m15_live`、`current_run_r3=trial_v2_20260303_r3_m20_e10_live` へドリフトしたため、3run を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260303_{h1_live,m15_live,r3_m20_e10_live}_refresh{2,2,1}_now1009.json` を生成。`tools/xau_autobot_live_loop_guard.py --include-r3 --dry-run` は `status=HEALTHY`（expected 3run 各1本）だが `unmanaged=xau_autobot.armada_mt5_20260303p1_pandajiro_01.json` を検出したため、`trial_v2_armada_20260303p1_pandajiro_1` も judge/audit 観測に接続し `xau_autobot_operational_audit_20260303_armada_pandajiro_refresh1_now1009.json` を追加。`v50_8_trial_judge_*`（h1/m15/r3_m20_e10/armada）・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260303_runtime_trial_v2_20260303_{v50_8_30d_h1_live,v50_8_30d_m15_live,r3_m20_e10_live}.json`・`v50_8_run_refresh_20260303_1010_h1live_m15live_m20e10_armada.json` を再同期。最新は managed 3run と armada の全てで `verdict=INVALID_TRIAL` / `closed_positions=0`、watchdog は managed 3run が `CONTINUE`、監査は全run `INSUFFICIENT_DATA`、月次判定は `decision=NO_GO` 継続。通信ポート/メッセージ契約は不変のため `INTERFACES.md` 更新なし。
- **2026-03-03**: V50.8-P5.107（option2 monitor-only refresh #26 / post-rotation sync）を反映。run meta の外部切替に追従し、`primary=trial_v2_20260303_primary_r2v_m5_48h` / `secondary=trial_v2_20260303_v50_8_30d_m15_live` / `explore=trial_v2_20260303_r3_entry48h_m5` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価。`xau_autobot_operational_audit_20260303_{primary_r2v_m5_48h,m15_live,r3_entry48h_m5}_refresh1_now0057.json` を生成し、`v50_8_trial_judge_*`（新3run）・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260303_runtime_trial_v2_20260303_{primary_r2v_m5_48h,v50_8_30d_m15_live}.json`・`v50_8_run_refresh_20260303_0958_r2v48h_m15live_r3entry48h.json` を再同期。`tools/xau_autobot_live_loop_guard.py --include-r3 --dry-run` は `status=HEALTHY`（expected 3run稼働）だが `unmanaged=xau_autobot.trial_v2_20260228_m20_executor_v1_m45_softgate_e10.json` を検出。最新は3runとも `verdict=INVALID_TRIAL`（`window_days≈0.00084/0.00093/0.00103`, `closed_positions=0`）、watchdogは全run `CONTINUE`、監査は `INSUFFICIENT_DATA/INSUFFICIENT_DATA/FAIL`、月次判定は `decision=NO_GO` 継続。通信ポート/メッセージ契約は不変のため `INTERFACES.md` 更新なし。
- **2026-03-03**: V50.8-P5.105（48h約定優先ピボット）を反映。`e10_shadow14d` の `closed_positions=0` 継続を受け、48時間は「約定母集団確保」を最優先にする契約へ更新。`primary` は約定実績のある `trial_v2_20260303_primary_r2v_m5_48h`、`secondary` は `trial_v2_20260303_v50_8_30d_m15_live`、`explore` は `trial_v2_20260303_r3_entry48h_m5` を正本runとする。48h判定は `closed_positions>=20 && profit_factor>=1.00 && net_profit>0` を採用し、未達runは fail-closed で次候補へ切替える。通信ポート/メッセージ契約は不変のため `INTERFACES.md` 更新なし。
- **2026-03-03**: V50.8-P5.106（48hピボット実行）を反映。`tools/configs/xau_autobot.trial_v2_20260303_r3_entry48h_m5.json` を追加し、run meta を `primary=trial_v2_20260303_primary_r2v_m5_48h` / `secondary=trial_v2_20260303_v50_8_30d_m15_live` / `explore=trial_v2_20260303_r3_entry48h_m5` へ更新。`tools/xau_autobot_live_loop_guard.py --include-r3` で `status=HEALTHY`（3run 各1本）へ収束。評価成果物 `xau_autobot_trial_judge_trial_v2_{20260303_primary_r2v_m5_48h,20260303_v50_8_30d_m15_live,20260303_r3_entry48h_m5}.json`、監査 `xau_autobot_operational_audit_20260303_{primary_r2v_m5_48h,m15_live,r3_entry48h_m5}_refresh{2,1,2}_now0102.json`、集約 `v50_8_run_refresh_20260303_0102_pivot48h.json` を生成。現時点は切替直後のため 3run とも `verdict=INVALID_TRIAL` / `closed_positions=0`、判定は `decision=NO_GO` 継続。
- **2026-03-03**: V50.8-P5.104（option2 monitor-only refresh #24）を反映。`tools/xau_autobot_live_loop_guard.py --include-r3` で managed trial 3本の `status=HEALTHY` を確認し、`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260302_r2_rotate_r2v_m5_001402` / `explore=trial_v2_20260302_r3_rotate_alpha4r2_001402` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価。`xau_autobot_operational_audit_20260303_{m20_e10_shadow14d,r2v_m5,alpha4_r2_liveprobe}_refresh{9,7,4}_now0052.json` を生成し、`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260303_0052_e10shadow_r2vm5_a4r2.json` を再同期。最新は `e10_shadow14d={window_days:2.3984190306944444, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED, audit:FAIL}`、`r2_rotate_r2v_m5_001402={window_days:1.0259386345949073, verdict:NO_GO, closed:21, watchdog:CONTINUE, audit:WARN}`、`r3_rotate_alpha4r2_001402={window_days:1.0261112166782407, verdict:NO_GO, closed:8, watchdog:CONTINUE, audit:FAIL}`。判定は `decision=NO_GO` 継続。
- **2026-03-03**: V50.8-P5.103（option2 monitor-only refresh #23）を反映。`tools/xau_autobot_live_loop_guard.py --include-r3 --dry-run` で `status=HEALTHY`（trial 3本稼働、`unmanaged=tuned_auto_active` 残存）を確認したうえで、`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260302_r2_rotate_r2v_m5_001402` / `explore=trial_v2_20260302_r3_rotate_alpha4r2_001402` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価。`xau_autobot_operational_audit_20260303_{m20_e10_shadow14d,r2_rotate_r2v_m5,r3_rotate_alpha4r2}_refresh{8,2,2}_now0050.json` を生成し、`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260302_runtime_20260302_r2_rotate_r2v_m5_001402.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260303_0950_e10shadow_r2rotate_r3rotate.json` を再同期。最新は `e10_shadow14d={window_days:2.396967727650463, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED, audit:FAIL}`、`r2_rotate_r2v_m5_001402={window_days:1.02441466875, verdict:NO_GO, closed:21, watchdog:CONTINUE, audit:WARN}`、`r3_rotate_alpha4r2_001402={window_days:1.024464914386574, verdict:NO_GO, closed:8, watchdog:CONTINUE, audit:FAIL}`。判定は `decision=NO_GO` 継続。
- **2026-03-02**: V50.8-P5.102（option2 monitor-only refresh #22 / live-loop recovery）を反映。`tools/xau_autobot_live_loop_guard.py --include-r3 --dry-run` で `status=DEGRADED`（trial 3本 missing、`xau_autobot.tuned_auto_active.json` 稼働）を確認後、`tools/xau_autobot_live_loop_guard.py --include-r3` で trial 3本を復旧（`status=HEALTHY`）。続けて `primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260302_r2_rotate_r2v_m5_001402` / `explore=trial_v2_20260302_r3_rotate_alpha4r2_001402` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260302_{m20_e10_shadow14d,r2_rotate_r2v_m5,r3_rotate_alpha4r2}_refresh{7,1,1}_now1354.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260302_runtime_20260302_r2_rotate_r2v_m5_001402.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260302_2255_e10shadow_r2rotate_r3rotate.json` を再同期。最新は `e10_shadow14d={window_days:1.942282118263889, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`、`r2_rotate_r2v_m5_001402={window_days:0.5697219975925926, verdict:INVALID_TRIAL, closed:0, watchdog:CONTINUE, audit:FAIL(gap_reject_rate=1.0)}`、`r3_rotate_alpha4r2_001402={window_days:0.5697665037037037, verdict:INVALID_TRIAL, closed:0, watchdog:CONTINUE, audit:FAIL(gap_reject_rate=1.0)}`。判定は `decision=NO_GO` 継続。
- **2026-03-02**: V50.8-P5.101（option2 monitor-only refresh #21）を反映。`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260302_r2_rotate_r2v_m5_001402` / `explore=trial_v2_20260302_r3_rotate_alpha4r2_001402` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260302_{m20_e10_shadow14d,r2v_m5,alpha4_r2_liveprobe}_refresh{8,6,3}_now1125.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260302_1126_e10shadow_r2vm5_a4r2.json` を再同期。最新は `e10_shadow14d={window_days:1.838348448125, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`、`r2_rotate_r2v_m5={window_days:0.4657425363425926, verdict:INVALID_TRIAL, closed:0, watchdog:CONTINUE, audit:FAIL}`、`r3_rotate_alpha4r2={window_days:0.46574241240740744, verdict:INVALID_TRIAL, closed:0, watchdog:CONTINUE, audit:FAIL}`。判定は `decision=NO_GO` 継続。
- **2026-03-02**: V50.8-P5.100（watchdog rotation execution）を反映。ユーザー選択 `1` に従い `secondary` を `trial_v2_20260302_r2_rotate_r2v_m5_001402`（`...r2v_m5.json`）、`explore` を `trial_v2_20260302_r3_rotate_alpha4r2_001402`（`...alpha4_r2_liveprobe.json`）へ切替。`tools/xau_autobot_live_loop_guard.py --include-r3` で新runを再起動し、旧 `h4_stable_tfscan/e10_c2` loop を停止後に `expected=3` / `unmanaged=0` の収束を確認。`xau_autobot_operational_audit_20260302_{m20_e10_shadow14d,r2v_m5,alpha4_r2_liveprobe}_refresh{7,5,2}_now0014.json` と `v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_trial_v2_20260302_r2_rotate_r2v_m5_001402.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260302_0915_e10shadow_r2vm5_a4r2.json` を再同期。最新は `primary=e10_shadow14d` が `watchdog:NO_GO_ROTATE_REQUIRED`、`secondary/explore` は rotation 直後で `watchdog:CONTINUE`。
- **2026-03-02**: V50.8-P5.99（option2 monitor-only refresh #20）を反映。`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260227_h4_stable_tfscan` / `explore=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_c2` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260302_{m20_e10_shadow14d,h4_stable_tfscan,m20_e10_c2}_refresh{6,9,9}_now0013.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260302_0914_e10shadow_h4stable_e10c2.json` を再同期。最新は `e10_shadow14d={window_days:1.37264636625, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`、`h4_stable_tfscan={window_days:1.8857027374074076, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`、`e10_c2={window_days:1.4390243451388889, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`。判定は `decision=NO_GO` 継続。
- **2026-03-01**: V50.8-P5.98（option2 monitor-only refresh #19）を反映。`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260227_h4_stable_tfscan` / `explore=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_c2` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260301_{m20_e10_shadow14d,h4_stable_tfscan,m20_e10_c2}_refresh{5,8,8}_now1441.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260301_2342_e10shadow_h4stable_e10c2.json` を再同期。最新は `e10_shadow14d={window_days:0.9747812644791667, verdict:INVALID_TRIAL, closed:0, watchdog:CONTINUE}`、`h4_stable_tfscan={window_days:1.4887872387384258, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`、`e10_c2={window_days:1.042074065625, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`。判定は `decision=NO_GO` 継続。
- **2026-03-01**: V50.8-P5.97（option2 monitor-only refresh #18）を反映。`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260227_h4_stable_tfscan` / `explore=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_c2` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260301_{m20_e10_shadow14d,h4_stable_tfscan,m20_e10_c2}_refresh{5,8,8}_now1439.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260301_2340_e10shadow_h4stable_e10c2.json` を再同期。最新は `e10_shadow14d={window_days:0.973453844525463, verdict:INVALID_TRIAL, closed:0, watchdog:CONTINUE}`、`h4_stable_tfscan={window_days:1.487728693113426, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`、`e10_c2={window_days:1.041102657488426, verdict:INVALID_TRIAL, closed:0, watchdog:NO_GO_ROTATE_REQUIRED}`。判定は `decision=NO_GO` 継続。
- **2026-03-01**: V50.8-P5.96（option2 monitor-only refresh #17）を反映。`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260227_h4_stable_tfscan` / `explore=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_c2` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260228_{m20_e10_shadow14d,h4_stable_tfscan,m20_e10_c2}_refresh{4,7,7}_now1708.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260301_0210_e10shadow_h4stable_e10c2.json` を再同期。最新は `e10_shadow14d={window_days:0.07660939278935186, verdict:INVALID_TRIAL, closed:0}`、`h4_stable_tfscan={window_days:0.5908392790162037, verdict:INVALID_TRIAL, closed:0}`、`e10_c2={window_days:0.14412638615740742, verdict:INVALID_TRIAL, closed:0}`。判定は `decision=NO_GO` 継続。
- **2026-03-01**: V50.8-P5.95（option2 monitor-only refresh #16）を反映。`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260227_h4_stable_tfscan` / `explore=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_c2` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260228_{m20_e10_shadow14d,h4_stable_tfscan,m20_e10_c2}_refresh{3,6,6}_now1701.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260301_0202_e10shadow_h4stable_e10c2.json` を再同期。最新は `e10_shadow14d={window_days:0.07196363021990741, verdict:INVALID_TRIAL, closed:0}`、`h4_stable_tfscan={window_days:0.5859697995601851, verdict:INVALID_TRIAL, closed:0}`、`e10_c2={window_days:0.1392570354861111, verdict:INVALID_TRIAL, closed:0}`。判定は `decision=NO_GO` 継続。
- **2026-03-01**: V50.8-P5.94（option2 monitor-only refresh #15）を反映。`primary=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_shadow14d` / `secondary=trial_v2_20260227_h4_stable_tfscan` / `explore=trial_v2_20260228_m20_executor_v1_m45_softgate_e10_c2` を `XAU_AUTOBOT_TRIAL_WATCHDOG_ENABLED=1` で再評価し、`xau_autobot_operational_audit_20260228_{m20_e10_shadow14d,h4_stable_tfscan,m20_e10_c2}_refresh{2,5,5}_now1529.json` を生成。`v50_8_trial_judge_*`・`v50_8_dual_run_status_20260226/20260227.json`・`v50_8_monthly_decision_20260225.json`・`v50_8_gap_analysis_20260226_runtime_20260228_m20_executor_v1_m45_softgate_e10_shadow14d.json`・`v50_8_gap_analysis_20260226_runtime_20260227_h4_stable_tfscan.json`・`v50_8_monitor_only_status_20260227.json`・`v50_8_run_refresh_20260301_0031_e10shadow_h4stable_e10c2.json` を再同期。最新は `e10_shadow14d={window_days:0.008339181527777777, verdict:INVALID_TRIAL, closed:0}`、`h4_stable_tfscan={window_days:0.5223451689814815, verdict:INVALID_TRIAL, closed:0}`、`e10_c2={window_days:0.07563205831018519, verdict:INVALID_TRIAL, closed:0}`。判定は `decision=NO_GO` 継続。
