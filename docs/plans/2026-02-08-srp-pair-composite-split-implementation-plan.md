# Pair-Composite SRP Split Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Split `school-pair-composite.lisp` into smaller files so SRP check passes (≤25 defuns per file) with no behavior change.

**Architecture:** Move the first 22 core utilities into `school-pair-composite-core.lisp` and the remaining selection/refresh logic into `school-pair-composite-selection.lisp`. Keep a tiny stub (or remove the old file) and update `swimmy.asd` load order to core → selection.

**Tech Stack:** Common Lisp (SBCL), ASDF system definition, existing SRP checker `tools/check_srp.py`.

### Task 1: Reproduce SRP Failure (Baseline)

**Files:**
- None

**Step 1: Run SRP check to confirm failure**

Run: `python3 tools/check_srp.py`

Expected: FAIL with `src/lisp/school/school-pair-composite.lisp: 30 functions (Max 25)`

**Step 2: Commit baseline note (optional)**

Skip commit; this is just a baseline check.

### Task 2: Create Core Pair-Composite File

**Files:**
- Create: `src/lisp/school/school-pair-composite-core.lisp`
- Modify: `src/lisp/school/school-pair-composite.lisp`

**Step 1: Write core file (no behavior change)**

Create `src/lisp/school/school-pair-composite-core.lisp` with the following content (moved verbatim from the current file):

```lisp
;;; school-pair-composite-core.lisp - Pair-Composite core utilities
...
**Step 2: Replace old file with a stub (no defuns)**

Update `src/lisp/school/school-pair-composite.lisp` to:

```lisp
;;; school-pair-composite.lisp - Deprecated stub (split into core/selection)

(in-package :swimmy.school)

;;; This file intentionally left without defun forms.
;;; See:
;;; - school-pair-composite-core.lisp
;;; - school-pair-composite-selection.lisp
```

**Step 3: Commit core split**

```bash
git add src/lisp/school/school-pair-composite-core.lisp src/lisp/school/school-pair-composite.lisp
```

### Task 3: Create Selection/Refresh File

**Files:**
- Create: `src/lisp/school/school-pair-composite-selection.lisp`

**Step 1: Write selection file (no behavior change)**

Create `src/lisp/school/school-pair-composite-selection.lisp` with the following content (moved verbatim from the current file):
...
**Step 2: Commit selection file**

```bash
git add src/lisp/school/school-pair-composite-selection.lisp
```

### Task 4: Update ASDF Load Order

**Files:**
- Modify: `swimmy.asd:118`

**Step 1: Replace the single file entry with core + selection**

Change:

```lisp
(:file "src/lisp/school/school-pair-composite")
```

To:

```lisp
(:file "src/lisp/school/school-pair-composite-core")
(:file "src/lisp/school/school-pair-composite-selection")
```

**Step 2: Commit ASDF change**

```bash
git add swimmy.asd
```

### Task 5: Verify SRP Passes

**Files:**
- None

**Step 1: Run SRP check**

Run: `python3 tools/check_srp.py`

Expected: `✅ All files compliant with SRP.`

**Step 2: (Optional) Run Lisp tests**
...
```bash
git commit -m "refactor: split pair-composite for SRP"
```
