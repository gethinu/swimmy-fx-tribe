...
SERVICE_UNIT="swimmy-forward-probe-watch.service"
TIMER_UNIT="swimmy-forward-probe-watch.timer"
SYSTEMCTL_FLAGS=()
...
echo "Install Forward Probe Watch systemd units"
...
if [[ "$SCOPE" == "system" ]] && [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
  echo "ERROR: Run as root (sudo) for system scope. Example: sudo bash tools/install_forward_probe_watch_service.sh" >&2
  exit 1
fi
