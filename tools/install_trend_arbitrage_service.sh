...
SERVICE_UNIT="swimmy-trend-arbitrage.service"
TIMER_UNIT="swimmy-trend-arbitrage.timer"
...
echo "Install Trend Arbitrage systemd units"
...
if [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
  echo "ERROR: Run as root (sudo). Example: sudo bash tools/install_trend_arbitrage_service.sh" >&2
  exit 1
fi
