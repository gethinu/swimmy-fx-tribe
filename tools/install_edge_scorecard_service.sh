...
SERVICE_UNIT="swimmy-edge-scorecard.service"
TIMER_UNIT="swimmy-edge-scorecard.timer"
SYSTEMCTL_FLAGS=()
...
echo "Install Edge Scorecard systemd units"
...
if [[ "$SCOPE" == "system" ]] && [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
  echo "ERROR: Run as root (sudo) for system scope. Example: sudo bash tools/install_edge_scorecard_service.sh" >&2
  exit 1
fi
